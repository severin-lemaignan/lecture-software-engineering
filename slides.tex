...
\begin{frame}{The staging area}
    ...
    \only<2>{
    To help preparing nice atomic commits!\\ $\Rightarrow$ No need to commit all current changes together.
    }
    \only<3->{
...
\begin{frame}{So far...}
    So far, we have looked at GIT from the only perspective of {\Medium code
    versioning}:
    \begin{itemize}
        \item {\Medium atomic commits} and how to prepare them;
        \item the 
    \end{itemize}
\end{frame}

\section{Viewed from a GUI...}

\begin{frame}{}
    \centering
    {\Medium GitHub for Windows} (GHfW) Walkthrough
    \vspace{2em}
...
\imageframe[black]{We swtich back to \texttt{numerical\_coordinates} and merge in \texttt{master}}{github-windows/13}
