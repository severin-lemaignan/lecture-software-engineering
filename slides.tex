...
\begin{frame}{Atomic commits}
    ...
    \only<2>{
    \begin{itemize}
        \item Easy to understand changes
...
\node at (-2, 1)[file,temporal=<3>{red}{red}{black}] (file1) {src/main.cpp};
\node [file,temporal=<1>{red}{red}{black},below=0.0 of file1.south east, anchor=north east] (file2) {src/position.cpp};
\node [file,temporal=<1>{red}{red}{black},below=0.02 of file2.south east,anchor=north east] (file3) {src/position.hpp};
\node [file,temporal=<2>{red}{red}{black},below=0.02 of file3.south east,anchor=north east] (file4) {share/model.csv};
    ...
    \path[draw] (file2.east) edge [->, bend left] (A);
    ...
    \onslide<3>{
        ...
        \path[draw] (file4.east) edge [->, bend left,hriSec2] (B);
    }
...
\texttt{git commit -m"Add cmdline opt to change simu. speed"}
}
...
Date:   Wed Feb 10 16:48:22 2016 +0000
    ...
    Add cmdline opt to change simu. speed
...
\begin{frame}{Branches}
    ...
    \begin{tikzpicture}
        ...
        \onslide<2>{
            ...
            \gitbranch
            {new-gui}     % node name and text 
            {above=of master} % node placement
            {master}          % target

            % HEAD reference
            \gitHEAD
            {above=of new-gui} % node placement
            {new-gui}          % target
        }
